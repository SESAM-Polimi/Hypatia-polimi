...
class BuildModel:
    ...
    def _solve_MO(self, number_solutions, path, verbosity, solver, **kwargs):
        ...
        if problem.status == "optimal":
            ...
            res.extend(to_add)
            result_1_collector = namedtuple("result_1", res)
            result_1 = result_1_collector(
                **{result_1: getattr(self.vars, result_1) for result_1 in res}
            )
            ...
            Max_emissions = result_1.tot_emissions.value
...
#-----------------------------------------------------------------------------------------------------
        ...
        if problem.status == "optimal":
            ...
            res.extend(to_add)
            result_2_collector = namedtuple("result_2", res)
            result_2 = result_2_collector(
                **{result_2: getattr(self.vars, result_2) for result_2 in res}
            )
            
            if self.model_data.settings.multi_node:
                Max_NPC = result_2.tot_cost_multi_node.value
                if self.model_data.settings.mode == ModelMode.Operation:
                    Max_NPC = result_2.tot_cost_multi_node.value[0]
            else:
                Max_NPC = result_2.tot_cost_single_node.value
                if self.model_data.settings.mode == ModelMode.Operation:
                    Max_NPC = result_2.tot_cost_single_node.value[0]
