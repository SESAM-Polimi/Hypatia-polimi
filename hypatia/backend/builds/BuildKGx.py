...
class BuildModel:
    ...
    def _solve_MO(self, number_solutions, path, verbosity, solver, **kwargs):
        ...
        step_npc = (Max_NPC-Min_NPC)/(number_solutions-1)  

        i = 1        
        NPC_list = []
        while i < (number_solutions-1):
             NPC_list.append(Max_NPC-i*step_npc)
             i += 1
        NPC_list = [Max_NPC] + NPC_list + [Min_NPC]
        emission_list = []
        printcount = 1
        for n in NPC_list:
            print("\n ------ NEW RUN Solution no. " + str(printcount) + " ------ " "\n")
            printcount +=1
            new_constr = [self.vars.totalcost_allregions <= n]
            objective = cp.Minimize(self.global_emission_objective)      
            problem = cp.Problem(objective, self.constr + new_constr)
            problem.solve(solver=solver, verbose=verbosity, **kwargs)
        ...
        ## emission_list = [Max_emissions] + emission_list[0:number_solutions-1] + [Min_emissions]
        ## NPC_list = [Min_NPC] + NPC_list[0:number_solutions-1] + [Max_NPC]
        
        # emission_list = emission_list[0:number_solutions]
        ...
        selected_npc_value = NPC_list[solution-1]
        print("Selected emission value = " + str(selected_npc_value))
        cost_constr = [self.vars.totalcost_allregions <= selected_npc_value]
        # emission_constr = [self.vars.tot_emissions == 844520166.0400091]
        if self.model_data.settings.mode == ModelMode.Operation and solution == number_solutions:
            objective = cp.Minimize(self.global_emission_objective)     
            problem = cp.Problem(objective, self.constr)
            ...
            # objective = cp.Minimize(self.global_objective)     
            objective = cp.Minimize(self.global_emission_objective)     
            problem = cp.Problem(objective, self.constr + cost_constr)
        problem.solve(solver=solver, verbose=verbosity, **kwargs)
